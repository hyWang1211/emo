...
%--------------------------------------------------------------------------
    ...
    properties
        add = cell(1,3);        % Additional properties of the solution
    end
    methods
        % original
        % function obj = SOLUTION(PopDec,PopObj,PopCon,PopAdd)
        % new
        function obj = SOLUTION(PopDec,PopObj,PopCon,PopAdd,celli)
    ...
    methods
        ...
        function value = adds(obj, Add, celli, update)
        %adds - Get or set the matrix of additional properties of multiple solutions.
        %
        %   Add = obj.adds(Add) returns the matrix of additional properties
        %   of multiple solutions obj. If any solution in obj does not
        %   contain an additional property, it will be set to the default
        %   value specified in Add.
            value = zeros(size(Add));
            for i = 1 : length(obj)
                if isempty(obj(i).add{celli}) || update == 1
                    obj(i).add{celli} = Add(i,:);
                end
                value(i, :) = obj(i).add{celli};
            end
            % original
            % value = cat(1,obj.add);
        end
